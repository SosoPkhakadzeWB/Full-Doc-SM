...
function renderSetterWorkflows() {
  const c = document.getElementById('setter-workflows');
  if (c) c.innerHTML = WB.workflows.filter(w => w.engine === 'setter').map(renderWorkflowCard).join('');
}
function renderShowRateWorkflows() {
  const c = document.getElementById('showrate-workflows');
  if (c) c.innerHTML = WB.workflows.filter(w => w.engine === 'showrate').map(renderWorkflowCard).join('');
}
function renderCloserWorkflows() {
  const c = document.getElementById('closer-workflows');
  if (c) c.innerHTML = WB.workflows.filter(w => w.engine === 'closer').map(renderWorkflowCard).join('');
}
function renderGlobalWorkflows() {
  const c = document.getElementById('showrate-workflows-global');
  if (c) c.innerHTML = WB.workflows.filter(w => w.engine === 'global').map(renderWorkflowCard).join('');
}
function renderAllWorkflows() {
  const c = document.getElementById('all-workflows-container');
  if (c) { c.innerHTML = WB.workflows.map(renderWorkflowCard).join(''); initWorkflowFilters(); }
}
...
function initWorkflowFilters() {
  document.querySelectorAll('.wf-filter').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      btn.classList.add('active'); filterWorkflows();
    });
  });
...
function renderSetterProperties() {
  const c = document.getElementById('setter-properties');
  if (!c) return;
  const groups = WB.properties.contact.filter(g =>
    g.group.includes('Setter') || g.group.includes('Speed') || g.group.includes('Core')
  );
  c.innerHTML = `<div class="prop-table-wrap">${groups.map(renderPropGroup).join('')}</div>`;
}
function renderShowRateProperties() {
  const c = document.getElementById('showrate-properties');
  if (c) c.innerHTML = `<div class="prop-table-wrap">${WB.properties.appointment.map(renderPropGroup).join('')}</div>`;
}
function renderCloserProperties() {
  const c = document.getElementById('closer-properties');
  if (c) c.innerHTML = `<div class="prop-table-wrap">${WB.properties.deal.map(renderPropGroup).join('')}</div>`;
}
function renderAllProperties() {
  const c = document.getElementById('all-properties-container');
  if (!c) return;
  const sections = [
  ...
  c.innerHTML = sections.map(s => `
    <div class="prop-section" data-obj="${s.obj}">
      <div style="font-family:var(--font-display);font-size:26px;font-weight:900;letter-spacing:1px;text-transform:uppercase;margin:32px 0 16px;padding-bottom:12px;border-bottom:2px solid var(--border-pink);">
...
function renderLeadStatusGrid() {
  const colorMap = { green:'badge-green', blue:'badge-blue', amber:'badge-amber', orange:'badge-orange', red:'badge-red', gray:'badge-gray' };
        ...
        <div class="dp-stages">
          ${group.stages.map(s => {
            ...
            const condHtml = s.conditionalOptions && s.conditionalOptions.length > 0 ? `
              <div style="margin-top:8px;padding:8px 10px;background:rgba(0,0,0,0.2);border-radius:6px;border-left:3px solid var(--pink)">
                <div style="font-size:11px;font-weight:700;color:var(--pink);margin-bottom:4px">📋 REQUIRED FIELDS:</div>
            ...
            return `
              <div class="dp-stage ${typeClass}">
                <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">
                  <span style="background:var(--bg4);border:1px solid var(--border2);border-radius:50%;width:22px;height:22px;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:800;flex-shrink:0">${stageNum}</span>
                ...
                ${badge}
                ${condHtml}
                ${s.triggers ? `<div style="font-size:11px;color:var(--pink);margin-top:6px;line-height:1.5">⚡ ${s.triggers}</div>` : ''}
              </div>
            `;
          }).join('')}
        </div>
    ...
    ${p.stages.map(group => `
      <div style="margin-bottom:24px">
        ...
        <div class="conf-pipeline">
          ${group.stages.map(s => {
            const borderColor = s.status === 'Closed' ? 'var(--text3)' : 'var(--pink)';
            const statusBadge = `<span class="${s.status === 'Open' ? 'badge-green' : 'badge-gray'}" style="font-size:10px">${s.status || 'Open'}</span>`;
            const condHtml = s.conditionalOptions && s.conditionalOptions.length > 0 ? `
              <div style="margin-top:8px;padding:8px 10px;background:rgba(0,0,0,0.2);border-radius:6px;border-left:3px solid var(--pink)">
                <div style="font-size:11px;font-weight:700;color:var(--pink);margin-bottom:4px">📋 HOW TO MOVE:</div>
                ${s.conditionalOptions.map(opt => `<div style="font-size:11px;color:var(--text2);padding:2px 0">• ${opt}</div>`).join('')}
              </div>` : '';
            return `
...
function renderPipelinesPage() {
  ...
  const renderStageCard = (s, idx, isDeal) => {
    ...
    if (s.type === 'auto') borderColor = 'var(--pink)';
    ...
    return `
      <div style="background:var(--bg3);border:1px solid var(--border);border-left:4px solid ${borderColor};border-radius:var(--radius-lg);padding:20px 24px;margin-bottom:12px">
        <div style="display:flex;align-items:flex-start;gap:16px;flex-wrap:wrap">
          <div style="min-width:180px">
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;flex-wrap:wrap">
              ${idx !== null ? `<span style="background:var(--bg4);border:1px solid var(--border2);border-radius:50%;width:26px;height:26px;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:800;flex-shrink:0">${idx}</span>` : ''}
          ...
          <div style="flex:1;min-width:220px">
            ...
            ${s.triggers ? `<div style="font-size:12px;color:var(--pink);line-height:1.6">⚡ <strong>What it triggers:</strong> ${s.triggers}</div>` : ''}
          </div>
          ${hasConditionals ? `
          <div style="min-width:260px;max-width:340px">
            <div style="background:rgba(255,45,120,0.06);border:1px solid var(--border-pink);border-radius:var(--radius);padding:14px">
              <div style="font-size:12px;font-weight:800;color:var(--pink);margin-bottom:10px;text-transform:uppercase;letter-spacing:0.5px">📋 Conditional Stage Properties</div>
              ...
              ${s.conditionalOptions.map(opt => `
                <div style="display:flex;align-items:flex-start;gap:6px;font-size:12px;color:var(--text1);padding:4px 0;border-bottom:1px solid rgba(255,255,255,0.05)">
                  <span style="color:var(--pink);flex-shrink:0;margin-top:2px">›</span><span>${opt}</span>
                </div>
              `).join('')}
            </div>
          </div>` : ''}
  ...
  const renderPipelineSection = (pipeline) => {
    ...
    return `
      <div class="pipeline-container-section">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:24px;padding:12px 16px;background:var(--bg3);border:1px solid var(--border);border-radius:var(--radius)">
        ...
        ${pipeline.stages.map(group => `
          <div style="margin-bottom:28px">
            ...
            ${group.stages.map(s => { stageNum++; return renderStageCard(s, isDeal ? stageNum : null, isDeal); }).join('')}
          </div>
        `).join('')}
      </div>
    `;
...
function renderListsPage() {
  ...
  const renderListCard = (l) => `
    <div class="list-card">
      ...
      <div class="list-desc" style="margin-bottom:10px">${l.desc}</div>
      ${l.url ? `<a href="${l.url}" target="_blank" rel="noopener" class="hs-link-btn">↗ Open in HubSpot</a>` : ''}
    </div>
  `;
...
function renderQuickLinksPage() {
  ...
  const svSection = WB.smartViews.map(cat => `
    <div class="ql-group">
      <div class="ql-group-label">
        ${cat.category === 'contact' ? '👤' : '💼'} Smart Views — ${cat.categoryLabel}
  ...
  container.innerHTML = `
    <div class="callout callout-info" style="margin-bottom:28px">
      <strong>🔗 All links open directly in HubSpot.</strong> Use the ↗ Open buttons to jump straight into the relevant view, dashboard, list, or workflow. Workflow links are also on each workflow card throughout this doc.
    </div>

    <h2 style="font-family:var(--font-display);font-size:32px;font-weight:900;letter-spacing:2px;text-transform:uppercase;margin-bottom:4px">Smart Views</h2>
    <p style="color:var(--text2);font-size:13px;margin-bottom:20px">Contact Object views (P1–P4, No-Shows, Confirmation Specialist) and Deal Object views (Red Zone, Nurture, Upcoming Appointments, At-Risk). Contact and Deal views are clearly labelled.</p>
    ${svSection}

    <h2 style="font-family:var(--font-display);font-size:32px;font-weight:900;letter-spacing:2px;text-transform:uppercase;margin-bottom:4px;margin-top:40px">Dashboards</h2>
    <p style="color:var(--text2);font-size:13px;margin-bottom:20px">Performance reporting dashboards for each engine.</p>
    ${dbSection}

    <h2 style="font-family:var(--font-display);font-size:32px;font-weight:900;letter-spacing:2px;text-transform:uppercase;margin-bottom:4px;margin-top:40px">Lead Scoring</h2>
    <p style="color:var(--text2);font-size:13px;margin-bottom:20px">The score that drives P1–P4 Smart View sort order.</p>
    ${lsSection}

    <h2 style="font-family:var(--font-display);font-size:32px;font-weight:900;letter-spacing:2px;text-transform:uppercase;margin-bottom:4px;margin-top:40px">Segments &amp; Lists</h2>
    <p style="color:var(--text2);font-size:13px;margin-bottom:20px">Active segments and static lists used across all engines. Click ↗ Open to inspect filters in HubSpot.</p>
    ${segSection}
  `;
}
        ...
        <div class="slack-channel">
          <div class="slack-ch-header">
            ...
            <div>
              ...
              <div style="font-size:12px;color:var(--text3);font-family:var(--font-mono)">${ch.id}</div>
            </div>
...
function renderIntegrationsPage() {
  ...
  container.innerHTML = `
    ...
    <div style="background:var(--bg3);border:1px solid var(--border);border-radius:var(--radius-lg);padding:24px;margin-top:24px">
      <div style="font-family:var(--font-display);font-size:22px;font-weight:900;text-transform:uppercase;letter-spacing:1px;margin-bottom:16px">🔄 How a Booking Flows Through All Integrations</div>
      <div style="display:flex;flex-direction:column;gap:12px">
        ${[
          ...
          <div style="display:flex;gap:12px;align-items:flex-start">
            <div style="width:28px;height:28px;background:linear-gradient(135deg,var(--pink),var(--purple));color:#fff;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:13px;flex-shrink:0">${s.step}</div>
            <div>
...
// ── SMART VIEW CARDS ON SETTER PAGE ──
function renderSetterSmartViewCards() {
  const container = document.getElementById('setter-smartview-cards');
  if (!container) return;
  const contactViews = WB.smartViews.find(sv => sv.category === 'contact');
  if (!contactViews) return;

  const priorityColors = ['sv-p1','sv-p2','sv-p3','sv-p4','sv-p4','sv-p4'];
  const priorityLabels = ['P1','P2','P3','P4','',''];

  container.innerHTML = contactViews.items.map((item, i) => `
    <div class="sv-card ${priorityColors[i] || 'sv-p4'}">
      <div class="sv-header">
        ${priorityLabels[i] ? `<span class="sv-priority">${priorityLabels[i]}</span>` : ''}
        <span class="sv-name">${item.name}</span>
      </div>
      <div class="sv-desc">${item.desc}</div>
      <a href="${item.url}" target="_blank" rel="noopener" class="sv-open-btn">↗ Open in HubSpot</a>
    </div>
  `).join('');
}
